...
from opacus.utils.module_modification import convert_batchnorm_modules, nullify_batchnorm_modules
...
def link_prediction(dataset, inductive, dim, model, rel_model, loss_fn,
    ...
    # model = nullify_batchnorm_modules(model)
    optimizer = Adam(model.parameters(), lr=lr)

    ALPHAS = [1 + x / 10.0 for x in range(1, 100)] + list(range(12, 64))
    DELTA = 1 / len(train_ent)
    NOISE_MULTIPLIER = 1
    MAX_GRAD_NORM = 1
    SAMPLE_RATE = batch_size / len(train_ent)
        ...
        for step, data in enumerate(train_loader):
            ...
            for p in model.parameters():
                if p.grad is not None:
                    p.grad += torch.normal(mean=0, std=NOISE_MULTIPLIER * MAX_GRAD_NORM, size=p.grad.shape) / batch_size
            ...
            if step % int(0.1 * len(train_loader)) == 0:
                # eps, alpha = optimizer.privacy_engine.get_privacy_spent(DELTA)
                eps, delta, alpha = compute_epsilon(steps=(epoch - 1) * total_steps + step + 1,
                                                    noise_multiplier=NOISE_MULTIPLIER,
                                                    sampling_probability=SAMPLE_RATE, alphas=ALPHAS, delta=DELTA)
